...
Viewer::Viewer() : Window(NULL), WindowWidth(0), WindowHeight(0), ViewportWidth(0), ViewportHeight(0) {}
...
void Viewer::HandleMouseInput()
{
	...
	if (MouseXCoordinate == std::numeric_limits<double>::max() || 
		MouseYCoordinate == std::numeric_limits<double>::max()) 
	{
	...
	float xOffset = static_cast<float> (xCoordinate - MouseXCoordinate);
	float yOffset = static_cast<float> (yCoordinate - MouseYCoordinate);
...
void Viewer::SetupScene() 
{
	...
	MeshMap["cube"]->PhysicRigidBody->LinearVelocity = glm::vec3(0.0f, -1.0f, 0);
	...
	MeshMap["bigfloor"]->PhysicRigidBody->SetState(
		glm::vec3(-1.0f, 14.0f, 0.0f), 
		glm::identity<glm::quat>(), s  
		glm::vec3(0.0f, 0.0f, 0.0f),
		glm::vec3(0.0f, 0.0f, 0.0f)
	);
	//MeshMap["bigfloor"]->PhysicRigidBody->LinearVelocity = glm::vec3(0.0f, -3.0f, 0);
	...
	physicEngine.SetTimeStep(1.0f / 60.0f);
...
bool Viewer::CheckMouseObjectsIntersection(Mesh3D& mesh, float& outDistance) {
